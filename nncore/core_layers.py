...
class SmartDense(nn.Module):
    ...
    def __init__(
        ...
        self.weight = nn.Parameter(TensorOps.create_tensor(
        ...
        if bias:
            self.bias = nn.Parameter(TensorOps.create_tensor(
                (out_features,),
                device=device,
                dtype=dtype
            ))
    ...
    def reset_parameters(self):
        ...
        WeightInitializer['kaiming_normal'](self.weight, mode='fan_out', nonlinearity='relu')
...
class AdvancedConv2d(nn.Module):
    ...
    def __init__(
        ...
        if separable and in_channels != groups:
            groups = in_channels
        ...
        WeightInitializer['kaiming_normal'](self.conv.weight, mode='fan_out', nonlinearity='relu')
        
        self.attention = None
        if attention:
            self.attention = nn.Sequential(
                nn.AdaptiveAvgPool2d(1),
                nn.Conv2d(out_channels, out_channels // 4, 1, device=device, dtype=dtype),
                nn.ReLU(inplace=True),
                nn.Conv2d(out_channels // 4, out_channels, 1, device=device, dtype=dtype),
                nn.Sigmoid()
            )
            # Initialize attention weights
            for m in self.attention.modules():
                if isinstance(m, nn.Conv2d):
                    WeightInitializer['kaiming_normal'](m.weight, mode='fan_out', nonlinearity='relu')
