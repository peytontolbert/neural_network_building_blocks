...
class MemoryModule(nn.Module):
    ...
    def __init__(
        ...
        self.candidate = nn.Sequential(nn.Linear(memory_dim * 2, memory_dim), nn.Tanh())

        # Move all components to the same device
        self.to(DeviceManager.get_device())

    def forward(
        self, query: torch.Tensor, mask: Optional[torch.Tensor] = None
        ...
        if mask is not None:
            ...
            attn_mask = torch.zeros(
                batch_size, self.num_heads, 1, self.memory_size, device=mask.device
            )
...
class DecisionLayer(nn.Module):
    ...
    def forward(
        ...
        std = torch.clamp(torch.exp(0.5 * log_var), min=self.min_std, max=self.max_std)
...
class PolicyNetwork(nn.Module):
    ...
    def forward(
        ...
        if self.discrete:
            ...
            std = torch.clamp(
                torch.exp(self.log_std), min=self.min_std, max=self.max_std
            )
...
class ValueEstimator(nn.Module):
    ...
    def __init__(
        ...
        self.feature_extractor = nn.Sequential(
            nn.Linear(state_dim, hidden_dim), nn.ReLU(), nn.Dropout(dropout)
        )
