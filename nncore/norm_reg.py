...
class AdaptiveLayerNorm(nn.Module):
    ...
    def __init__(
        ...
        if adaptive_elementwise_affine:
            ...
            self.gate = nn.Parameter(TensorOps.create_tensor(
                torch.zeros(normalized_shape),
                device=device,
                dtype=dtype
            ))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.adaptive_elementwise_affine:
            ...
            scale = self.weight * gate + (1.0 - gate)  # Interpolate between weight and 1
...
class PopulationBatchNorm(nn.Module):
    ...
    def __init__(
        ...
        if self.track_running_stats:
            self.register_buffer('running_mean', TensorOps.create_tensor(
                torch.zeros(num_features),
                device=device,
                dtype=dtype
            ))
            self.register_buffer('running_var', TensorOps.create_tensor(
                torch.ones(num_features),
                device=device,
                dtype=dtype
            ))
...
class StructuredDropout(nn.Module):
    ...
    def __init__(
        ...
        if adaptive:
            self.dropout_prob = nn.Parameter(TensorOps.create_tensor(
                torch.tensor(p),
                device=device
            ))
