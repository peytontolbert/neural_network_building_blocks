...
def demonstrate_working_memory_buffer():
    ...
    buffer = WorkingMemoryBuffer(buffer_size=buffer_size, memory_dim=memory_dim)
...
def demonstrate_hierarchical_memory():
    ...
    for i, (old, new) in enumerate(
        zip(old_memories, [level.memory.data for level in memory.levels])
    ):
...
def demonstrate_shared_swarm_memory():
    ...
    print(
        f"Shared memory updated: {not torch.equal(memory.shared_memory.data, old_memory)}"
    )
...
class MemoryAugmentedNetwork(nn.Module):
    ...
    def __init__(
        ...
        self.episodic = EpisodicMemory(
            memory_size=memory_size, memory_dim=hidden_dim, query_dim=hidden_dim
        )

        self.working = WorkingMemoryBuffer(
            buffer_size=memory_size // 2, memory_dim=hidden_dim
        )
    ...
    def forward(self, x):
        ...
        combined = torch.cat(
            [episodic_out, working_out, hierarchical_out, swarm_consensus], dim=-1
        )
