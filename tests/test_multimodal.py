...
def test_speech_encoder():
    ...
    expected_dim = hidden_dim * len(encoder.conv_layers[0])  # hidden_dim * num_kernel_sizes
...
def test_vision_processor():
    ...
    expected_sizes = [(height // 2**(i+2), width // 2**(i+2)) for i in range(processor.num_scales)]
