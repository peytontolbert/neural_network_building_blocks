...
def test_memory_module():
    ...
    memory = MemoryModule(
        memory_size=memory_size, memory_dim=memory_dim, query_dim=query_dim
    )
...
def test_decision_layer():
    ...
    layer = DecisionLayer(input_dim=input_dim, output_dim=output_dim)
...
def test_state_encoder():
    ...
    encoder = StateEncoder(input_dim=input_dim, hidden_dim=hidden_dim)
...
def test_policy_network():
    ...
    discrete_policy = PolicyNetwork(
        state_dim=state_dim, action_dim=action_dim, discrete=True
    )
    ...
    continuous_policy = PolicyNetwork(
        state_dim=state_dim, action_dim=action_dim, discrete=False
    )
...
def test_value_estimator():
    ...
    estimator = ValueEstimator(state_dim=state_dim, ensemble_size=ensemble_size)
    ...
    assert not torch.allclose(
        values[0], values[1]
    )  # Different ensemble members should give different predictions
