...
def test_enhanced_residual_block():
    ...
    block_stride = EnhancedResidualBlock(
        channels,
        stride=2,
        downsample=nn.Conv2d(channels, channels * 4, kernel_size=1, stride=2, device=device),
        device=device
    )
...
def test_feature_pyramid_block():
    ...
    block_no_residual = FeaturePyramidBlock(in_channels, out_channels, use_residual=False, device=device)
...
def test_dynamic_routing_block():
    ...
    block_temp = DynamicRoutingBlock(channels, num_experts=num_experts, temperature=0.5, device=device)
    output_temp, routing_weights_temp = block_temp(x)
    
    assert output_temp.shape == (batch_size, channels, height, width)
    assert not torch.equal(routing_weights, routing_weights_temp)  # Different temperature should give different weights
